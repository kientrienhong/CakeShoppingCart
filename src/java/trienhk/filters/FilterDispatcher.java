...
public class FilterDispatcher implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
            FilterChain chain)
            throws IOException, ServletException {
        logger = Logger.getLogger(FilterDispatcher.class.getName());
        ...
        if (uri.contains(".jpg") || uri.contains(".png") || uri.contains(".gif")) {
            ...
            try {
                ...
                if (url == null) {
                    ...
                    if (session != null) {
                        TblRegistrationDTO dto = (TblRegistrationDTO) session.getAttribute("DTO");
                        if (dto != null) {
                            TblRoleDAO roleDAO = new TblRoleDAO();
                            String roleName = roleDAO.getNameById(dto.getRoleId());
                            req.setAttribute("NAME_ROLE", roleName);
                        }
                    }
                    if (!uri.contains("Member") && !uri.contains("Admin")) {
                ...
                if (t.getMessage().contains("Can't find resource")) {
                    ((HttpServletResponse) response).sendRedirect("");
                }
                logger.error(t);
            }
